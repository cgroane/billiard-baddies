...
export const getPoolTables = async () => {
  ...
  const data = await user.mongoClient("mongodb-atlas").db("pool-tables").collection('tables').find();
...
//   const data = await user.mongoClient("mongodb-atlas").db("pool-tables").collection('tables')"tables").find();
